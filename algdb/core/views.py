...
class CreateTemplateAlgView(View):
    ...
    def post(self,request):
        obj=models.AlgTemplate(name=request.POST["algname"],desc=request.POST["algdesc"])
        obj.save()
        for i in request.POST:
            if(i.startswith("op")):
                obj.strategies.add(int(request.POST[i]))
            elif(i.startswith("param")):
                obj.params.add(int(request.POST[i]))
        obj.save()
        return HttpResponseRedirect(reverse('talg', args=[obj.id]))

class CreateAlgView(View):
    def get(self, request):
        if("id" in request.GET):
            obj=models.AlgTemplate.objects.get(id=int(request.GET["id"]))
            return render(request,'algcreate.html',{"talg":obj})
        else:
            objs=models.AlgTemplate.objects.all()
            if("algname" in self.request.GET) and (self.request.GET["algname"]!=""):
                objs=objs.filter(name__icontains=self.request.GET["algname"])
            return render(request, 'algtemplate_selection.html',{"algs":objs})
    def post(self,request):
        obj=models.Algorithm(template=models.AlgTemplate.objects.get(id=int(request.GET["id"])))
        obj.save()
        for i in request.POST:
            if i.startswith("param_"):
                param=models.AlgParam.objects.get(id=int(i.split("_")[1]))
                poa=models.ParamOfAlg(algorithm=obj,param=param,**{models.typeIdToFieldStr(param.type):models.stringToValue(request.POST[i],param.type)})
                poa.save()
        obj.save()
